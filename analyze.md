...
**Fix Applied** (verified against current implementation):
1. `first()` sets `LIMIT 1` before delegating to `get()` and restores the caller's limit in a `finally` block — only a single row is hydrated instead of wrapping an entire result set in a Collection.
2. The `$softDeleteScopeApplied` guard intentionally persists after the call: the applied soft-delete constraint stays consistent with the builder's persistent wheres, so reusing the same builder never duplicates or drops the constraint, and fresh queries always start from fresh builders. State management around multi-step reads is handled at the right level: `paginate()` saves/restores the guard (plus wheres/bindings/selects) around its internal `count()` so the data query re-applies the scope freshly, and `chunk()` restores it in a `finally`.
...
| # | Description |
|---|-------------|
| 1 | ~~`increment()`/`decrement()` on Model instance: if the column is cast as `'int'`, the in-memory sync may produce floating-point values.~~ Fixed by reading the current value through `castGet()` (so `'int'` casts yield real PHP ints) instead of force-casting to `(float)`, coercing to float only for non-numeric values; the synced value now keeps its cast type even when a mutator bypasses `castSet()`. |
| 2 | ~~`paginate()` calls `count()` which calls `applySoftDeleteScope()`, then restores state — the guard is correctly reset so `get()` re-applies the scope. Similarly, `first()` now saves/restores the guard.~~ **Verified** — `paginate()` saves/restores `$softDeleteScopeApplied` (plus wheres/bindings/selects) around its internal `count()` (QueryBuilder.php:3897-3911), and `chunk()` restores it in a `finally`. `first()` delegates to `get()` and intentionally leaves the guard set: the applied constraint remains consistent with the builder's persistent wheres, so builder reuse never duplicates or loses the soft-delete scope. |
| 3 | ~~`chunk()` / `each()` modify and restore `$this->limit`/`$this->offset`/`$this->softDeleteScopeApplied` — if an exception occurs during callback, state may be corrupted.~~ Fixed. The loop already ran inside a `try/finally`, but the restore was incomplete: it reset the soft-delete guard while the constraints added by `applySoftDeleteScope()` remained in `$this->wheres`, leaving an inconsistent builder (a follow-up `get()` would re-apply the scope onto wheres that already contained it). `chunk()` now snapshots and restores `$wheres`/`$bindings` alongside limit/offset/guard in the `finally`, so the builder returns to its exact pre-call state on any exit path; `each()` delegates to `chunk()` and inherits the guarantee. |
| 4 | ~~`belongsTo()` with null FK: returns an always-empty query via `whereIn($ownerKey, [])` — but eager loading doesn't apply this same guard.~~ **Verified** — resolved by the eager-loading refactor (commit a5bd335). The lazy path compiles `whereIn($ownerKey, [])` to a valid always-false `0=1` predicate (QueryBuilder.php:594-599), and every belongsTo eager path now applies the equivalent guard: `eagerLoadRelation()` (`with()`), `loadRelationCount()` (`withCount()`, → 0), `loadRelationAggregate()` (`withSum/Avg/Min/Max` → null) all filter null FKs from the batch keys and short-circuit with no query when empty, mapping null-FK models back to `null`; `eagerLoadTouchedRelations()` (`$touches`) does the same per relation. The correlated `EXISTS` subqueries used by `whereHas()`/`has()` need no guard — a null outer FK makes the comparison NULL, so the row is excluded, matching lazy semantics. |
| 5 | ~~`upsert()` with empty `$values` returns `0` but doesn't validate column consistency across rows.~~ Fixed. The empty-batch `return 0` is kept (matching Eloquent), but both `Model::upsert()` and `QueryBuilder::upsert()` now call the new `Helpers::validateConsistentColumns($values, ...)` after row normalization: every row must carry exactly the same columns as the first row (order-insensitive) or an `InvalidArgumentException` names the offending index and both key sets. Previously only `$values[0]` defined the INSERT column list, so later rows with missing keys were silently inserted as `NULL` (via `$row[$col] ?? null`) and extra keys silently dropped — silent data corruption. The validator also rejects non-array rows with a clear message instead of a fatal `array_keys()` error. |

### PHP Errors / Warnings

| # | Severity | File | Description |
|---|----------|-----------|-------------|
| 1 | ~~**Medium**~~ **Fixed** | Model.php | ~~`new static` inside `ensureTableExists()` when the table check guard fails to prevent recursion creates infinite loop potential.~~ Fixed by adding `resolveTableName()` static method. |
| 2 | ~~**Medium**~~ **Fixed** | QueryBuilder.php | ~~`new static($this->model)` in `join()` closure creates a new QueryBuilder with global scopes enabled — should be `false`.~~ Fixed by passing `false` as the constructor's `$applyGlobalScopes` argument (QueryBuilder.php:1960). Previously the sub-builder received global scopes and its wheres were then spliced into the JOIN's ON clause — e.g. the soft-delete scope would constrain *which joined rows match* instead of filtering results in WHERE (an INNER JOIN would silently drop parent rows whose related row was soft-deleted). Now the closure-built ON conditions contain only what the user adds; global scopes keep applying to the outer query only, matching Eloquent's JoinClause behavior. `leftJoin()`/`rightJoin()` delegate to `join()`, so all closure joins are covered. |
| 3 | ~~**Low**~~ **Fixed** | Model.php | ~~`fill()` logs a warning for blocked attributes but doesn't throw — silent failure may hide bugs.~~ Fixed by throwing `MassAssignmentException::forAttribute($key, static::class)` instead of logging, matching Eloquent's behavior and the existing `__set()` semantics. This also covers every path that funnels through `fill()`: the constructor (`new Model([...])`), `create()`, `firstOrCreate()`/`firstOrNew()`/`updateOrCreate()`. Verified: blocked attributes throw from all these paths while `forceFill()` still bypasses the guards deliberately. |
| 4 | ~~**Low**~~ **Fixed** | ColumnDefinition.php | ~~`addslashes()` on default values — doesn't handle multibyte characters correctly for MySQL.~~ Fixed by adding `Helpers::escapeSqlString()` — an encoding-independent escaper (backslashes doubled, single quotes doubled, NUL as `\0`) — and routing all DDL string-literal embedding through it: `ColumnDefinition::toSql()` DEFAULT literals plus `Blueprint::enum()`/`set()` values (the same bug existed there). Verified byte-level: with GBK, old `addslashes` output `0xBF 0x5C 0x27` merges into one multibyte char followed by a live quote, while quote-doubling (`''`) cannot be merged by any charset. Valid UTF-8 passes through untouched, and raw keyword defaults (CURRENT_TIMESTAMP etc.) are unaffected. |
...
| `$model->fill()` | ✅ Compatible (throws MassAssignmentException on guarded attributes, like Eloquent) |
...
| `$query->without()` / `Model::without()` (eager load exclusion) | ✅ Compatible |
...
| 5 | ~~`Model::without()` (eager load exclusion)~~ | **Implemented** | ✅ `QueryBuilder::without($relations)` removes relation(s) from the eager-load set — handling both storage forms (`with('posts')` numeric entries and `with(['posts' => constraint])` string-keyed entries) — plus a static `Model::without()` passthrough; instance calls work through the existing `__call` proxy. Excluded relations are dropped before any query runs. |
...
6. ~~**Fix `first()` soft delete scope state management**~~ — **Fixed** — `first()` now applies `LIMIT 1` around a single delegated `get()` call (one row hydrated), and soft-delete guard state is managed by `paginate()` (save/restore around its internal `count()`) and `chunk()` (`finally` restore).
...
| **Phase 2: Core Eloquent Parity** | ~~Add forceFill~~ (Implemented), ~~setAppends~~ (Implemented), ~~Collection::filter()~~ (Implemented), ~~without~~ (Implemented), getAttributes, isClean, syncOriginal | 3-5 days |
...
All previously reported critical and high-severity issues have been fixed: `update()` sending all attributes and not syncing original state, static state sharing in `ensureTableExists()`, `__callStatic` incompatibility with PHP 8.x property access, `first()` soft delete scope state management, `whereColumn()` SQL injection, `__set()` silent mass-assignment guard failure, `resolvePageFromRequest()` page manipulation, custom cast instantiation crash with non-existent classes, and `getTable()` double-prefixing. Additionally, the `whereExists` regex has been fixed to handle `SELECT DISTINCT`, `SELECT SQL_CALC_FOUND_ROWS`, and other SQL keywords between `SELECT` and the column list, `Collection::firstOrFail()` now throws a meaningful `ModelNotFoundException` with the correct model class name, the runtime `append()` / `setAppends()` methods are implemented, the `increment()`/`decrement()` in-memory sync no longer produces floating-point values for `'int'`-cast columns, `chunk()` now fully restores builder state (limit/offset plus soft-delete scope wheres/bindings/guard) even when the callback throws, belongsTo eager loading applies the same null-FK guard as the lazy path (null keys filtered from batched queries, no query issued when none remain), `upsert()` validates that all rows share a consistent column set instead of silently inserting NULLs or dropping keys, the join-closure sub-builder no longer receives global scopes so scope constraints stay in the outer WHERE instead of corrupting JOIN semantics, `fill()` now throws a `MassAssignmentException` for blocked attributes instead of logging a warning — making every mass-assignment path (`fill()`, constructor, `create()`, `updateOrCreate()`, etc.) fail loudly like Eloquent — and DDL string literals (column defaults, ENUM/SET values) are escaped with a charset-safe helper instead of `addslashes()`, and `Model::without()` / `QueryBuilder::without()` now provide Eloquent-style eager-load exclusion (usable statically, on the builder, or through the instance proxy). The architecture could benefit from splitting the 2700-line Model.php into focused traits, and the project would greatly benefit from a test suite and static analysis.
